...
public:
    int findRadius(vector<int> &houses, vector<int> &heaters)
    {
        ...
        {
            int minDistance = helper(house, heaters);
...
private:
    int helper(int house, const vector<int> &heaters)
    {
        int left = 0;
        ...
        while (right - left >= 2)
        {
            ...
            int m = abs(house - heaters.at(middle));
            int l = abs(house - heaters.at(middle - 1));
            int r = abs(house - heaters.at(middle + 1));
            if (m < l && m < r)
                return m;
        ...
        if (right - 1 == left)
            return min(abs(house - heaters[left]), abs(house - heaters[right]));
...
TEST_F(LeetCodeTest, Failure1)
{
    ...
    EXPECT_EQ(solution.findRadius(houses, heaters), 0);
}
